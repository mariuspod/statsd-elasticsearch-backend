...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  elasticHost           = process.env.ES_HOST            || configEs.host           || 'localhost';
  elasticPort           = process.env.ES_PORT            || configEs.port           || 9200;
  elasticPath           = process.env.ES_PATH            || configEs.path           || '/';
  elasticIndex          = process.env.ES_INDEX_PREFIX    || configEs.indexPrefix    || 'statsd';
  elasticIndexTimestamp = process.env.ES_INDEX_TIMESTAMP || configEs.indexTimestamp || 'day';
  elasticCountType      = process.env.ES_COUNT_TYPE      || configEs.countType      || 'counter';
  elasticTimerType      = process.env.ES_TIMER_TYPE      || configEs.timerType      || 'timer';
  elasticTimerDataType  = process.env.ES_TIMER_DATA_TYPE || configEs.timerDataType  || elasticTimerType + '_stats';
  elasticGaugeDataType  = process.env.ES_GAUGE_DATA_TYPE || configEs.gaugeDataType  || 'gauge';
  elasticFormatter      = process.env.ES_FORMATTER       || configEs.formatter      || 'default_format';
  elasticUsername       = process.env.ES_USERNAME        || configEs.username       || undefined;
  elasticPassword       = process.env.ES_PASSWORD        || configEs.password       || undefined;

  lg.log("debug", "elasticHost: " + elasticHost);
